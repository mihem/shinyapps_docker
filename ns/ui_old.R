...
shinyUI <- fluidPage(
    ...
    radioButtons("disruption", "Blood-brain barrier disruption?",
       ...
       sliderInput("hladrcd4", label = "% HLA-DR+ CD4+ cells (CSF):",
        ...
        sliderInput("csf_plasma", label = "% Plasma cells (CSF):",
    ...
    mainPanel(h1("Differentiating neurosarcoidosis from multiple sclerosis", align = "center"), br(),
              ...
              h4(textOutput("ns")),
              h4(textOutput("ms")), br(),
              em("Note: this model is experimental and has not been approved for clinical use. Based on a small cohort (24 NS patients, 49 MS patients) its estimated sensitivity is 94% and specificity 72% (MS defined as positives, NS defined as negatives). More information can be found in the following paper: Heming et al.")
    )
    )
)
