...
##----------------------------------------------------------------------------##
output[["spatial_projection_group_filters_UI"]] <- renderUI({
  group_filters <- list()
  for ( i in getGroups() ) {
    group_filters[[i]] <- shinyWidgets::pickerInput(
      paste0("spatial_projection_group_filter_", i),
      label = i,
...
outputOptions(
  output,
  "spatial_projection_group_filters_UI",
...
##----------------------------------------------------------------------------##
observeEvent(input[["spatial_projection_group_filters_info"]], {
  showModal(
    modalDialog(
      spatial_projection_group_filters_info[["text"]],
      title = spatial_projection_group_filters_info[["title"]],
      easyClose = TRUE,
      footer = NULL,
      size = "l"
    )
...
## Text in info box.
##----------------------------------------------------------------------------##
spatial_projection_group_filters_info <- list(
...
# output[["spatial_projection_group_filters_tree"]] <- shinyTree::renderTree({
#   groups <- list()
#   for ( i in getGroups() ) {
#     groups[[i]] <- structure(
#       as.list(
...
# output[["spatial_projection_group_filters_selected_groups"]] <- renderPrint({
#   tree <- input[["spatial_projection_group_filters_tree"]]
#   req(spatial_projection_group_filters_tree)
#   str(shinyTree::get_selected(tree, format = "slices"))
# })

# output[["spatial_projection_group_filters_tree_UI"]] <- renderUI({
#   tagList(
#     shinyTree::shinyTree(
#       "spatial_projection_group_filters_tree",
#       theme = "proton",
#       themeIcons = FALSE,
#       themeDots = FALSE,
#       checkbox = TRUE
