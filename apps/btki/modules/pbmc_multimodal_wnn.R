  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$plot_box <- renderUI({
      ...
      box(
        ...
        solidHeader = TRUE,
        width = NULL,
        plotOutput(ns("density_plot"),  height = paste0(plot_height, "px"))  # ✅ 用 ns()
      )
    })
...
pbmc_adt_rna_cell_type_corr_viz_Server <- function(id, data_list) {
  moduleServer(id, function(input, output, session) {
    ...
    output$plot_box <- renderUI({
      lang_val <- lang()
      title_suffix <- if (!is.null(lang_val) && length(lang_val) > 0 && lang_val == "cn") "RNA-ADT 相关性热图说明" else "RNA-ADT Heatmap Explanation"
      ...
      box(
        ...
        solidHeader = TRUE,
        width = NULL,
        plotOutput(ns("rna_adt_corr_heatmap"),  height = paste0(plot_height, "px"))  # ✅ 用 ns()
      )
    })

    # Render the plot based on selected cell type
    output$rna_adt_corr_heatmap <- renderPlot({
      # Check if input and data are available
      ...
      validate(need(!is.null(selected_data), "No data available for the selected cell type."))

      # 色标范围
      symmetric_limits <- TRUE  # 若 TRUE，填色范围对称于 0
      midpoint <- 0  # 填色梯度中点 (默认 0)
      cor_method <- "spearman"  # 相关系数方法，"spearman" / "pearson"
      facet_ncol <- 4  # 每行子图数
      ...
      palette_mid <- "white"
      palette_high <- "red"
      label_digits <- 2  # 小数显示位数

      cor_vals <- selected_data$cor
      if (all(is.na(cor_vals))) {
        warning("所有相关系数均为 NA，可能是数据缺失或方差为 0。")
        cor_min <- -1
        cor_max <- 1
      } else {
      ...
      # 确保 xlab, ylab, title 是字符向量
      xlab <- "RNA"
      ylab <- "ADT"
      title <- "RNA–ADT Marker Correlation by Cluster"

      # 确保 cluster 列存在且为字符类型
      ...
      #     legend.text = element_text(size = plot_params()$legend_text_size),
      #     legend.title = element_text(size = plot_params()$legend_title_size),
      #     strip.text = element_text(size = plot_params()$axis_text_size, face = "bold"),
