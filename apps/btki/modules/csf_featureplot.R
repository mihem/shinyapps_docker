...
csf_featureplot_UI <- function(id) {
  ns <- NS(id)
  ...
  fluidRow(
    ...
    column(
      ...
      box(
        ...
        # Add button group
        tags$div(
          id = ns("button_group"),  # Add container ID
...
csf_featureplot_Server <- function(id, umap_data, expr_df) {
  moduleServer(id, function(input, output, session) {
    ...
    output$sampling_ui <- renderUI({
      ...
      nearest100 <- function(x) {
        if (n_total < 100) return(n_total)
        y <- round(x / 100) * 100
        y <- max(100, min(y, n_total))
        y
      }
      sizes_raw <- c(n_total/20, n_total/5, n_total/2, n_total)
    ...
    # Sampled UMAP data
    sampled_umap <- reactive({
      req(n_total)
      size <- as.integer(input$sample_cells)
      if (is.null(size) || is.na(size) || size >= n_total) {
        return(umap_data)
      }
      set.seed(1)  # Remove set.seed for random sampling each time
      umap_data[sample.int(n_total, size), c("cell", "umap_1", "umap_2", "celltype_merged.l1"), drop = FALSE]
    })
    ...
    # Process user-entered gene list
    ...
    tile_base <- 350
    tile_gap  <- 40   # Estimate (legend + padding) to make column calculation conservative, adjustable
    ncol_dynamic_raw <- reactive({
      req(input$window_size)
      w <- input$window_size$width
      if (is.null(w) || is.na(w)) return(2)
      # Right box occupies 9/12 of total width, but we get window width -> can estimate right area ~ w * 0.70
      avail <- w * 0.70
    ...
    # Dynamic UI: dynamically set plot height based on gene count
    ...
    output$featureplot_plot <- renderPlot({
      # print("Rendering feature plot featureplot_plot")

      current_umap <- sampled_umap()   # ← Use sampled UMAP

      # print(current_umap[1:5, 1:3])
      genes <- selected_genes()
      ...
      # Verify expression matrix contains all sampled cells
      missing_cells <- setdiff(cells_needed, colnames(expr_df))
      validate(need(length(missing_cells) == 0,
                    sprintf("Expression matrix missing %d sampled cells", length(missing_cells))))

      # Extract gene expression in current_umap cell order (rows=genes, cols=cells)
      expr_filtered <- expr_df[genes, cells_needed, drop = FALSE]
      validate(need(nrow(expr_filtered) > 0, "No expression data for genes."))

      expr_mat <- t(as.matrix(expr_filtered)) # Transpose to (rows=cells, cols=genes), row names remain as cells
      ...
      # Direct column bind, no join needed (cbind aligns by row order)
      umap_data_with_expr <- cbind(
        current_umap,
        as.data.frame(expr_mat, check.names = FALSE)  # Avoid escaping gene names
      )

      # Optional: remove large temporary object references to speed up GC
